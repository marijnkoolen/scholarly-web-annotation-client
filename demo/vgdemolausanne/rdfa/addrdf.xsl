...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
        ...
        <processes>
            <process>
                <step path="tei:div[@type='original']" typeofw="vg:Letter" typeofr="hi:EditionText"
                    ...
                    replacementPattern="http://www.example.org/vgcorr/letter=$2/"
                    replacementPatternr="http://www.example.org/vgedition/vgcorr/letter=$2/repr=$1/"/>
                <step path="tei:ab" typeofw="vg:ParagraphInLetter" typeofr="hi:EditionText"
                    ...
                    replacementPattern="http://www.example.org/vgcorr/letter=$2/para=$3/"
                    replacementPatternr="http://www.example.org/vgedition/vgcorr/letter=$2/para=$3/repr=$1/"
                />
            </process>
            <process>
    ...
    <xsl:template match="/">
        <xsl:result-document href="let001plusrdfa.xml" method="xml" use-character-maps="num2ent">
            <xsl:text>&#10;</xsl:text>
            <saxon:doctype>
                ...
                <rdf:RDF
                    ...
                    <xsl:for-each select="$processes//process">
                        <xsl:call-template name="process">
                            <xsl:with-param name="stepnum" select="1" as="xs:integer"/>
...
<!-\-        <xsl:message>path <xsl:value-of select="$startpath"/></xsl:message>-\->
        ...
        <xsl:for-each select="$xpathout/*">
            ...
            <xsl:call-template name="vg:writettlline">
                ...
                <xsl:with-param name="o">
                    <xsl:value-of select="$process/step[$stepnum]/@typeofw"/>
                </xsl:with-param>
            </xsl:call-template>
            <xsl:call-template name="vg:writettlline">
                <xsl:with-param name="s">
                ...
                <xsl:with-param name="p">rdf:type</xsl:with-param>
                <xsl:with-param name="o">
                    <xsl:value-of select="$process/step[$stepnum]/@typeofr"/>
                </xsl:with-param>
            </xsl:call-template>
            <xsl:call-template name="vg:writettlline">
                <xsl:with-param name="s">
                ...
                <xsl:with-param name="p">
                    <xsl:value-of select="$process/step[$stepnum]/@association"/>
                </xsl:with-param>
                <xsl:with-param name="o">
                    <xsl:value-of select="vg:enclose($urls[2])"/>
                </xsl:with-param>
            ...
            <xsl:if test="$parenturis">
                <xsl:call-template name="vg:writettlline">
                    ...
                    <xsl:with-param name="p">
                        <xsl:value-of select="$process/step[$stepnum]/@propertyw"/>
                    </xsl:with-param>
                    <xsl:with-param name="o">
                        <xsl:value-of select="vg:enclose($urls[1])"/>
                    </xsl:with-param>
                ...
                <xsl:call-template name="vg:writettlline">
                    ...
                    <xsl:with-param name="p">
                        <xsl:value-of select="$process/step[$stepnum]/@propertyr"/>
                    </xsl:with-param>
                    <xsl:with-param name="o">
                        <xsl:value-of select="vg:enclose($urls[2])"/>
                    </xsl:with-param>
    ...
    <xsl:template name="vg:writettlline">
        ...
        <rdf:Description about="{vg:entity-abbrev(vg:unenclose($s))}">
            <xsl:attribute name="{$p}" select="vg:entity-abbrev(vg:unenclose($o))"/>
        </rdf:Description>
    </xsl:template>
